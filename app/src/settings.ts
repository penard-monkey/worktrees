...
// ── column geometry ─────────────────────────────────────────────────────────
// The window is rail + nav + main + dock. Only `main` is elastic, so both side
// panels need a ceiling derived from the CURRENT viewport — a flat cap either
...
export const navCeiling = (dockW = 0, w = viewport()) =>
  Math.max(NAV_MIN, Math.min(NAV_MAX, w - RAIL_W - MAIN_MIN - dockW));
/** Widest the dock may be while `navW` of nav is on screen. */
export const dockCeiling = (navW = 0, w = viewport()) =>
  Math.max(DOCK_MIN, w - RAIL_W - navW - MAIN_MIN);
...
export function fitLayout(s: Settings, dockEligible: boolean, w = viewport()): Fit {
  ...
  if (dockShown && w - RAIL_W - navW - dockW < MAIN_MIN) {
  ...
  const mainW = w - RAIL_W - navW - dockW;
