...
@test "close: kills an ADOPTED session (different name, pane cwd'd in the worktree)" {
  mk_wt feat-x
  # simulate `open` having adopted a foreign session: rename the canonical one
  wt_phys="$(cd "$REPO/.worktrees/feat-x" && pwd -P)"
  rm -f "$TMUX_STATE/repo-feat-x"
  printf 'cwd=%s\ncmd0=bash\n' "$wt_phys" > "$TMUX_STATE/scratch-session"
